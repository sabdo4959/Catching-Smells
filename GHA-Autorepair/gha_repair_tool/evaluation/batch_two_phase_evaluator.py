...
class TwoPhaseEvaluator(BaselineEvaluator):
    """2단계 평가 클래스 (베이스라인 평가 상속)"""
    
    def __init__(self, output_dir: str = "./evaluation_results/two_phase"):
        """
        Args:
...
class TwoPhaseBatchProcessor:
    ...
    def __init__(self, output_dir: str = "./evaluation_results/two_phase"):
        ...
        # 평가 결과를 저장할 디렉토리 (2단계는 같은 레벨에)
        self.evaluation_dir = self.output_dir
        
        self.evaluator = TwoPhaseEvaluator(str(self.evaluation_dir))
    
    def evaluate_two_phase_files(self, original_dir: str, repaired_dir: str, 
        ...
        for original_file in original_files:
            # 2단계 복구된 파일 경로 생성
            repaired_file = repaired_path / f"{original_file.name}_two_phase_repaired.yml"
        ...
        self.logger.info(f"2단계 평가 시작: {total_files}개 파일 쌍")
        ...
        self.logger.info(f"2단계 평가 완료: {processing_time:.1f}초")
        
        return batch_summary
    
    def evaluate_file_list(self, file_pairs: List[Tuple[str, str]], 
                          group_name: str = "two_phase_test") -> Dict[str, any]:
        """
        파일 쌍 리스트를 직접 평가합니다.
        ...
        """
        total_files = len(file_pairs)
        self.logger.info(f"2단계 파일 리스트 평가 시작: {total_files}개 파일 쌍")
        ...
        self.logger.info(f"2단계 파일 리스트 평가 완료: {processing_time:.1f}초")
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="2단계 배치 평가 도구")
    
    parser.add_argument("--original-dir", required=True, help="원본 파일 디렉토리")
    parser.add_argument("--repaired-dir", required=True, help="2단계 복구된 파일 디렉토리")
    parser.add_argument("--max-files", type=int, help="평가할 최대 파일 수")
    parser.add_argument("--output-dir", default="./evaluation_results/two_phase", help="출력 디렉토리")
    ...
    try:
        processor = TwoPhaseBatchProcessor(args.output_dir)
        
        summary = processor.evaluate_two_phase_files(
            args.original_dir, 
            args.repaired_dir, 
            args.max_files
        )
        ...
        logging.error(f"2단계 평가 중 오류 발생: {e}")
        sys.exit(1)
